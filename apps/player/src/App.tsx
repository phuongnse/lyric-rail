...
function Thumbnail({ item, selected, lyrics }: { item: LibraryItem; selected: boolean; lyrics?: string }) {
  ...
  if (selected) return <div className="song-thumbnail lyric-thumbnail" aria-label={`${item.title} lyrics`}><p>{lyrics || item.firstLyricLine || "No lyrics yet"}</p></div>;
...
type DrawerProps = {
  ...
  onAiProcess?: (item: LibraryItem) => void;
  onOpenActivity?: (item: LibraryItem) => void;
            ...
            <div className="virtual-space" style={{ height: props.items.length * ROW_HEIGHT }}>
              {visible.map((item, index) => {
                ...
                return (
                  ...
                  >
                    <Thumbnail item={item} selected={props.selectedId === item.id} lyrics={props.selectedId === item.id ? props.selectedLyrics : undefined} />
...
function App() {
  ...
  const contextFocusTarget = () => {
    ...
    return active?.closest(".row-actions")?.querySelector<HTMLElement>('[aria-label^="Open actions for"]') ?? active;
  };
  ...
  const replayTaskOutput = useCallback((taskId: string) => {
    ...
    void (async () => {
      try {
        ...
        reportError("tasks", "Task output could not be replayed", reason, undefined, undefined, taskId);
      } finally {
  ...
  useEffect(() => {
    ...
    if (!native) {
      setSelectedLyrics(selectedItem.firstLyricLine || selectedItem.lyricSnippet || "");
      return;
    }
    let disposed = false;
    invoke<string>("item_lyrics", { itemId: selectedItem.id })
      .then((text) => { if (!disposed) setSelectedLyrics(text); })
      .catch(() => { if (!disposed) setSelectedLyrics(selectedItem.firstLyricLine || ""); });
    return () => { disposed = true; };
  }, [native, selectedItem]);
  ...
  const runBusy = async (
    ...
    setBusy(true);
    try { await action(); }
    catch (reason) { reportError(scope, title, reason, undefined, undefined, relatedTaskId); }
  ...
  const prepareClip = async (path: string, compatible = false) => {
    ...
    try {
      ...
      if (!request.cancelled) {
        if (!compatible) setClipDialogOpen(false);
        else setClipPreparationError("Compatible preview failed. Your sections are kept; you can retry.");
        throw reason;
      }
    } finally {
  ...
  const addFiles = () => {
    ...
    }, "library", "Files could not be added", "clip-preparation");
  };
  ...
  const commitClips = async (sections: ClipSection[]) => {
    ...
    setClipBusy(true);
    try {
      let snapshot = await invoke<CatalogSnapshot>("commit_local_sections", { clipId: clipPreview.clipId, sections });
      for (const [index, section] of sections.entries()) {
        const lyrics = section.lyrics?.trim() ? section.lyrics : undefined;
        const item = snapshot.items[index];
        if (!lyrics || !item) continue;
        try {
  ...
  const processLibraryItem = (item: LibraryItem) => {
    setSelectedId(item.id);
    if (item.status === "waiting-for-lyrics") {
      showLyricDialog(item, "add").catch((reason) => reportError("lyrics", "Lyric editor could not open", reason));
      return;
    }
    const activeTask = taskState.tasks.find((task) => task.id === item.id && (task.status === "queued" || task.status === "running" || task.status === "paused"));
    if (activeTask) {
      showActivityTask(activeTask);
      return;
    }
    void runBusy(
      () => invoke("retry_processing_item", { itemId: item.id }),
      "processing",
      `AI processing could not start for ${item.title}`,
      item.id,
    );
  };

  const showLyricDialog = async (item: LibraryItem, mode: "add" | "edit") => {
    lyricRestoreRef.current = contextFocusTarget();
  ...
  const copyIssueDiagnostics = async (issue: SystemIssue) => {
    if (!navigator.clipboard) {
      reportError("issues", "Diagnostics could not be copied", "Clipboard access is unavailable");
      return;
    }
    const knownTasks = [...taskState.tasks];
    if (selectedTaskRecord && !knownTasks.some((task) => task.id === selectedTaskRecord.id)) {
      knownTasks.push(selectedTaskRecord);
    }
    if (native && issue.relatedTaskId && !knownTasks.some((task) => task.id === issue.relatedTaskId)) {
      try {
        const relatedTask = await invoke<TaskRecord | null>("task_record", { taskId: issue.relatedTaskId });
        if (relatedTask) knownTasks.push(normalizeTaskRecord(relatedTask));
      } catch {
        // The issue and runtime state remain useful when the bounded task record has expired.
      }
    }
    const tasks = selectDiagnosticTasks(issue, knownTasks);
    const taskReports = await Promise.all(tasks.map(async (task) => {
      let lines = taskState.output[task.id] ?? [];
      let truncated = Boolean(taskOutputTruncated[task.id]) || task.outputTruncated;
      if (native) {
        try {
          const snapshot = await invoke<TaskOutputSnapshot>("task_output_snapshot", {
            taskId: task.id,
          ...
          // Use the already-received ring-buffer lines if replay is unavailable.
        }
      }
      return { task, lines, truncated };
    }));
    ...
    try {
      ...
      reportError("issues", "Diagnostics could not be copied", reason);
    }
  };
  ...
  const openIssueTask = async (issue: SystemIssue) => {
    ...
    if (!task && native) {
      try {
        ...
        reportError("tasks", "Linked task output could not be opened", reason, undefined, undefined, taskId);
        return;
      }
    }
    if (task) {
  ...
  const showItemContext = async (item: LibraryItem) => {
    ...
    if (task && (task.status === "queued" || task.status === "running" || task.status === "paused")) showActivityTask(task);
  ...
  const copyTaskOutput = () => {
    ...
    if (!navigator.clipboard) {
      reportError("tasks", "Task output could not be copied", "Clipboard access is unavailable", undefined, undefined, selectedTaskId);
      return;
    }
    ...
    navigator.clipboard.writeText(report)
      .catch((reason) => reportError("tasks", "Task output could not be copied", reason, undefined, undefined, selectedTaskId));
  };
  ...
  return (
    <main className="app-shell">
      ...
      <LibraryDrawer
        ...
        onAiProcess={processLibraryItem}
        onOpenActivity={showItemContext}
